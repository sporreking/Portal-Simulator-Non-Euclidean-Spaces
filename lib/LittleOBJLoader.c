...
// 210304: Corrected bad allocation size on row 538, found by Jens Lindgren.
...
static void GenerateNormals(Mesh *mesh) {
    ...
    if (mesh->vertices && !mesh->vertexNormals) {
        ...
        for (face = 0; face * 3 < mesh->coordCount; face++) {
            ...
            float angle0 = (influence0 >= 1.f) ? 0 : (influence0 <= -1.f) ? M_PI
                                                                          : acos(influence0);
            float angle1 = (influence1 >= 1.f) ? 0 : (influence1 <= -1.f) ? M_PI
                                                                          : acos(influence1);
            float angle2 = (influence2 >= 1.f) ? 0 : (influence2 <= -1.f) ? M_PI
                                                                          : acos(influence2);
